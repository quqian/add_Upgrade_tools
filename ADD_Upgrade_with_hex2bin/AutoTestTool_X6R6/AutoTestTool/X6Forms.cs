...
namespace AutoTestTool
{
    public partial class X6Forms : Form
    {
        ...
        int UpGradeValue = 0;
        int UpGradeFinishFlag = 0;
        int UpGradeSendPercent = 0;

        private void X6Form1_Load(object sender, EventArgs e)
        {
            X6progressBar.Maximum = 100;
            X6progressBar.Value = 0;
        ...
        private void X6Form1_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (X6UPGradeThread != null)
                {
                    if (X6UPGradeThread.IsAlive)
                    {
                        X6UPGradeThread.Abort();
                        X6UPGradeThread = null;
                    }
                }
        ...
        private void X6skinButton_WholeChg_StartTest_Click(object sender, EventArgs e)
        {
            ...
            if (X6textBox_WholeChg_SN_QR.Text.IndexOf(ProcTestData.StationIdQrcodeUrl) == 0)
            {
                ...
                X6textBox_WholeChg_SN_QR.Text = X6textBox_WholeChg_SN_QR.Text.Remove(0, ProcTestData.StationIdQrcodeUrl.Length);
        ...
        private void X6skinButtWWWWWWWWWWon_CHG_TRUMPT_SUCCESS_Click(object sender, EventArgs e)
        {
        ...
        public Dictionary<string, string> X6ModifyResultData(Dictionary<string, string> inputDic)
        {
            ...
            while (X6SBTestingFlag == true)
            {
                ...
                switch (X6SBTabSelectIndex)
                {
                    ...
                    case 0x04://结束测试
                        ...
                        X6SBTestResultDir = X6ModifyResultData(X6SBTestResultDir);
        ...
        {
            X6progressBar.Value = (int)UpGradeValue;
            pStrUpgrade = UpGradeValue.ToString("D2") + "%";
            X6skinLabel_UpGrade.Text = pStrUpgrade;

            if (UpGradeFinishFlag == 0xa5)
            {
                UpGradeFinishFlag = 0;
                MessageBox.Show("升级已完成!!!");
            }
            if (X6tick++ >= 10)
            {
                X6RtcCount++;
        ...
        private void X6button_GetUPgradeData_Click(object sender, EventArgs e)
        {
           ...
           // file.Filter = @"升级包|*.bin";
            ...
            {
                X6textBox_UPgrade.Text = file.FileName;
                filename = file.FileName;
                X6textBoxConfigPrint.AppendText("已加载升级包！\n");
            }
        }
        ...
        public byte[] ConvertToBinary(string Path)
        {
            ...
            //pStr += stream.Length.ToString();
            // LOG_UPGrade("文件大小:" + pStr);
            //LOG_UPGrade("文件大小qqqq:" + stream.Length);
            ...
            //    checkSum += buffer[k];
            //     LOG_UPGrade("h" + k);
            //    LOG_UPGrade("k"  + buffer[k].ToString("X2"));
            //   }
            //LOG_UPGrade("校验码为:" + checkSum);

            return buffer;
        }

        //升级线程
        string X6qqqfilepath = @".\X6_Upgrade_Tools_V1.1\aba.bin";  //这个BIN文件必须由keil编译出来
        private void X6StartUpGradeProcess()
        {
            int cout = 0, len = 0;
            long k = 0;
       //     FileStream fs = new FileStream(X6qqqfilepath, FileMode.Create);
            FileStream fs = new FileStream(X6qqqfilepath, FileMode.Open);
            //     FileStream fs =File.Create(X6qqqfilepath);                      
       ...
       //     byte[] sendbuf = ConvertToBinary(filename);
            byte[] sendbufHead = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
      //      len = sendbuf.Length;
            LOG_UPGrade("长度大小:" + len);
            
            try
            {
                bw.Write(sendbufHead);
                for (k = 0; k < 8192; k++)
                {
                 //   bw.Write(sendbufHead[0]);
                    //    bw.Write(sendbuf);
                }
                bw.Close();
                fs.Close();
         ...
         //       else
           ...
           //     LOG_UPGrade(""+ sendbuf[k].ToString("X2"));
                
        //        UpGradeValue = (int)(k * 100 / sendbuf.Length);
          //      UpGradeSendPercent = (int)(k / sendbuf.Length);               
       //     }
        //    UpGradeFinishFlag = 0xa5;
        }

        private void X6button_StartUpGrade_Click(object sender, EventArgs e)
        {
            if (X6textBox_UPgrade.Text == "")
            {
            ...
            X6UPGradeThread = new Thread(X6StartUpGradeProcess);
            X6UPGradeThread.Start();
        }

        private void X6button_StopUpGrade_Click(object sender, EventArgs e)
        {
            if(X6UPGradeThread != null)
            {
                X6UPGradeThread.Abort();
